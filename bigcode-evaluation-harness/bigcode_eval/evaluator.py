...
class Evaluator:
    def __init__(self, accelerator, model, tokenizer, tokenizer_enc, args):
    ...
    def generate_text(self, task_name, load_data_path):
        task = tasks.get_task(task_name, load_data_path, self.args)
    ...
    def evaluate(self, task_name, load_data_path):
        task = tasks.get_task(task_name, load_data_path, self.args)
        if task.requires_execution and not self.allow_code_execution:
            raise ValueError(_WARNING)

        generations, references = self.generate_text(task_name, load_data_path)

        if self.accelerator.is_main_process:
            if not self.args.load_generations_path:
                if self.args.save_generations:
                    Path(self.args.save_generations_path).parent.mkdir(
